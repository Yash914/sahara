...
app = FastAPI(title="SAHARA Mental Health Distress Monitoring API")
...
model = xgb.XGBRegressor(); model.load_model(str(BASE_DIR / "model" / "xgboost_distress_model.json")); explainer = shap.TreeExplainer(model)
...
@app.get("/")
def health(): return {"status":"online","app":"SAHARA"}
...
async def analyze_interaction(case_id:str=Form(...),text:str=Form(""),audio:UploadFile|None=File(None)):
    ...
    if has_audio:
        ...
        except subprocess.CalledProcessError: raise HTTPException(400,"Could not convert uploaded audio. The audio file may be invalid or unsupported.")
