...
emotion_model = AutoModelForSequenceClassification.from_pretrained(str(MODEL_DIR), local_files_only=True)
emotion_model.to(DEVICE)
emotion_model.eval()
emotion_id2label = {int(k): str(v).lower() for k, v in emotion_model.config.id2label.items()}
...
def analyze_text(text: str):
    ...
    batch = emotion_tokenizer(text, return_tensors='pt', truncation=True, max_length=256)
    batch = {key: value.to(DEVICE) for key, value in batch.items()}
    logits = emotion_model(**batch).logits[0]
    probs = torch.softmax(logits, dim=-1).cpu().numpy()
    probabilities = {emotion_id2label[i]: float(probs[i]) for i in range(len(probs))}
    dominant_emotion = max(probabilities, key=probabilities.get)
    return {
        'dominant_emotion': dominant_emotion,
        'confidence': round(float(probabilities[dominant_emotion]), 4),
