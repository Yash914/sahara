...
class _LoginState extends State<LoginScreen> {
  ...
  void enter() {
    ...
    if (authority) {
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => AuthorityScreen(logout: backToLogin)));
    } else {
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => ParticipantScreen(caseId: value, logout: backToLogin)));
    }
  }
  void backToLogin() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(body: Center(child: SingleChildScrollView(padding: const EdgeInsets.all(24), child: ConstrainedBox(
      ...
      child: Card(child: Padding(padding: const EdgeInsets.all(26), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Center(child: Column(children: [
          ...
          const SizedBox(height: 10), const Text('SAHARA', style: TextStyle(fontSize: 30, fontWeight: FontWeight.w800, color: navy)),
          const Text('Support • Assessment • Human Assistance • Response', textAlign: TextAlign.center, style: TextStyle(fontSize: 12)),
        ])),
        const SizedBox(height: 28), const Text('SECURE ACCESS', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: green, letterSpacing: 1.2)),
        ...
        const SizedBox(height: 20), Text(authority ? 'Authority ID' : 'Case ID', style: const TextStyle(fontWeight: FontWeight.w600)),
        const SizedBox(height: 7), TextField(controller: id, decoration: const InputDecoration(border: OutlineInputBorder(), prefixIcon: Icon(Icons.badge_outlined))),
        ...
        const SizedBox(height: 20), SizedBox(width: double.infinity, height: 52, child: FilledButton.icon(onPressed: enter, icon: const Icon(Icons.arrow_forward), label: Text(authority ? 'Open authority console' : 'Continue securely'))),
        const SizedBox(height: 14), const Row(children: [Icon(Icons.verified_user_outlined, size: 16, color: green), SizedBox(width: 8), Expanded(child: Text('Privacy-first access with human oversight.', style: TextStyle(fontSize: 12)))])
      ])))
    )));
  }
}
...
class _ParticipantState extends State<ParticipantScreen> {
  ...
  Future<void> toggleRecord() async {
    ...
    final dir = await getTemporaryDirectory(); final path = '${dir.path}/sahara_${DateTime.now().millisecondsSinceEpoch}.wav';
  ...
  Future<void> submit() async {
    ...
    catch (e) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Could not submit: $e'))); }
    finally { if (mounted) setState(() => busy = false); }
  }
  @override void dispose() { timer?.cancel(); recorder.dispose(); text.dispose(); super.dispose(); }
  @override Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(title: const Text('SAHARA'), actions: [IconButton(onPressed: widget.logout, icon: const Icon(Icons.logout))]), body: ListView(padding: const EdgeInsets.all(18), children: [
...
class _AuthorityState extends State<AuthorityScreen> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('Authority Console'), actions: [IconButton(onPressed: load, icon: const Icon(Icons.refresh)), IconButton(onPressed: widget.logout, icon: const Icon(Icons.logout))]), bottomNavigationBar: NavigationBar(selectedIndex: tab, onDestinationSelected: (i) => setState(() => tab = i), destinations: const [NavigationDestination(icon: Icon(Icons.dashboard_outlined), selectedIcon: Icon(Icons.dashboard), label: 'Overview'), NavigationDestination(icon: Icon(Icons.people_outline), selectedIcon: Icon(Icons.people), label: 'Cases')]), body: loading ? const Center(child: CircularProgressIndicator()) : ListView(padding: const EdgeInsets.fromLTRB(16, 12, 16, 30), children: [
      ...
      if (tab == 0) ...[
        _StatGrid(total: cases.length, urgent: urgent, increasing: increasing, interactions: interactions), const SizedBox(height: 22), const Text('Cases needing review', style: TextStyle(fontSize: 19, fontWeight: FontWeight.bold)), const SizedBox(height: 10),
        if (cases.isEmpty) const _Empty(title: 'No active cases yet', subtitle: 'Cases will appear after a participant submits a check-in.'),
        for (final raw in cases.where((x) => x['risk'] != 'Low' || x['trend'] == 'Increasing')) _CaseTile(data: Map<String, dynamic>.from(raw), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => CaseDetailScreen(caseId: raw['case_id'].toString()))))
      ],
      if (tab == 1) ...[
        TextField(onChanged: (v) => setState(() => query = v), decoration: const InputDecoration(prefixIcon: Icon(Icons.search), hintText: 'Search Case ID', border: OutlineInputBorder())), const SizedBox(height: 14),
        if (filtered.isEmpty) const _Empty(title: 'No matching cases', subtitle: 'Try another Case ID.'),
        for (final raw in filtered) _CaseTile(data: Map<String, dynamic>.from(raw), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => CaseDetailScreen(caseId: raw['case_id'].toString()))))
      ]
    ]));
  }
}
...
class _CaseDetailState extends State<CaseDetailScreen> {
  ...
  Future<void> load() async { try { final results = await Future.wait([ApiService.getSummary(widget.caseId), ApiService.getHistory(widget.caseId)]); final s = results[0] as Map<String, dynamic>; final h = results[1] as Map<String, dynamic>; if (mounted) setState(() { summary = s; history = List<dynamic>.from(h['history'] ?? []); loading = false; }); } catch (e) { if (mounted) { setState(() => loading = false); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Unable to load case: $e'))); } } }
  ...
  @override void initState() { super.initState(); load(); }
  @override Widget build(BuildContext context) {
    final d = summary ?? <String, dynamic>{}; final score = (d['current_score'] as num?)?.toDouble() ?? 0; final risk = (d['current_risk'] ?? '-').toString(); final trend = (d['trend'] ?? '-').toString(); final priority = (d['priority'] ?? '-').toString(); final urgent = risk == 'High' || risk == 'Severe' || trend == 'Increasing';
